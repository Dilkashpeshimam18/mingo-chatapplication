...
const ProfileOptions = () => {
    ...
    const handleDelete = async () => {
        try {
            ...
            const response = await reqInstance.delete(`http://13.53.118.65:4000/room/delete-room/${id}`)
                .then(() => {
                    alert('Room Deleted!')
                    dispatch(roomActions.handleDefault())
    ...
    const handleLeaveRoom = async () => {
        try {
            ...
            const res = reqInstance.delete(`http://13.53.118.65:4000/room/leave-room/${id}`)
            dispatch(roomActions.handleDefault())
